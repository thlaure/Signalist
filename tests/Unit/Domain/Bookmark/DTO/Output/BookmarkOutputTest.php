<?php

declare(strict_types=1);

namespace App\Tests\Unit\Domain\Bookmark\DTO\Output;

use App\Domain\Bookmark\DTO\Output\BookmarkOutput;
use App\Entity\Article;
use App\Entity\Bookmark;
use App\Entity\Category;
use App\Entity\Feed;
use DateTimeImmutable;
use PHPUnit\Framework\TestCase;
use Symfony\Component\Uid\Uuid;

final class BookmarkOutputTest extends TestCase
{
    public function testFromEntityMapsAllFields(): void
    {
        $categoryId = Uuid::v7();
        $category = $this->createMock(Category::class);
        $category->method('getId')->willReturn($categoryId);
        $category->method('getName')->willReturn('Tech');

        $feedId = Uuid::v7();
        $feed = $this->createMock(Feed::class);
        $feed->method('getId')->willReturn($feedId);
        $feed->method('getTitle')->willReturn('Example Feed');
        $feed->method('getCategory')->willReturn($category);

        $articleId = Uuid::v7();
        $article = $this->createMock(Article::class);
        $article->method('getId')->willReturn($articleId);
        $article->method('getTitle')->willReturn('Article Title');
        $article->method('getUrl')->willReturn('https://example.com/article');
        $article->method('getFeed')->willReturn($feed);

        $bookmarkId = Uuid::v7();
        $createdAt = new DateTimeImmutable('2025-02-01T12:00:00+00:00');

        $bookmark = $this->createMock(Bookmark::class);
        $bookmark->method('getId')->willReturn($bookmarkId);
        $bookmark->method('getNotes')->willReturn('Interesting read');
        $bookmark->method('getCreatedAt')->willReturn($createdAt);
        $bookmark->method('getArticle')->willReturn($article);

        $output = BookmarkOutput::fromEntity($bookmark);

        $this->assertSame($bookmarkId->toRfc4122(), $output->id);
        $this->assertSame('Interesting read', $output->notes);
        $this->assertSame('2025-02-01T12:00:00+00:00', $output->createdAt);
        $this->assertSame($articleId->toRfc4122(), $output->articleId);
        $this->assertSame('Article Title', $output->articleTitle);
        $this->assertSame('https://example.com/article', $output->articleUrl);
        $this->assertSame($feedId->toRfc4122(), $output->feedId);
        $this->assertSame('Example Feed', $output->feedTitle);
        $this->assertSame($categoryId->toRfc4122(), $output->categoryId);
        $this->assertSame('Tech', $output->categoryName);
    }

    public function testFromEntityWithNullNotes(): void
    {
        $category = $this->createMock(Category::class);
        $category->method('getId')->willReturn(Uuid::v7());
        $category->method('getName')->willReturn('Tech');

        $feed = $this->createMock(Feed::class);
        $feed->method('getId')->willReturn(Uuid::v7());
        $feed->method('getTitle')->willReturn('Feed');
        $feed->method('getCategory')->willReturn($category);

        $article = $this->createMock(Article::class);
        $article->method('getId')->willReturn(Uuid::v7());
        $article->method('getTitle')->willReturn('Title');
        $article->method('getUrl')->willReturn('https://example.com');
        $article->method('getFeed')->willReturn($feed);

        $bookmark = $this->createMock(Bookmark::class);
        $bookmark->method('getId')->willReturn(Uuid::v7());
        $bookmark->method('getNotes')->willReturn(null);
        $bookmark->method('getCreatedAt')->willReturn(new DateTimeImmutable());
        $bookmark->method('getArticle')->willReturn($article);

        $output = BookmarkOutput::fromEntity($bookmark);

        $this->assertNull($output->notes);
    }
}
